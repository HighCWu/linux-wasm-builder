...
Nix重型构建使用仓库范围的GitHub Actions cache，并以Nix表达式和`flake.lock`作为缓存键。
wasm64和常规CI使用独立的分层键：常规CI可从wasm64缓存恢复共享工具链，但完成后必须写入
包含wasm32用户态的自身缓存。生产者任务在保存前将store限制为9 GiB，并必须为LLVM等
昂贵的build-only依赖保留GC root；矩阵消费者只恢复常规CI缓存，不得为每个测试项
写入重复缓存。缓存未命中时的首次构建会较慢，但不得为了提速而把缓存当作发布物或
跳过源码可重建性检查。
